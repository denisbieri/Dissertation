"VAR4" <-
  function (y, p = 1, type = c("const", "trend", "both", "none"),
            season = NULL, exogen = NULL, lag.max = NULL, ic = c("AIC", "HQ", "SC", "FPE"), taus)
  {
    y <- as.matrix(y)
    if (any(length(taus)!=ncol(y)))
      stop("\nNumber of taus differ from number of variables.\n")
    if (any(is.na(y)))
      stop("\nNAs in y.\n")
    if (ncol(y) < 2)
    ...
    for (i in 1:K) {
      y <- yend[, i]
      equation[[colnames(yend)[i]]] <- rq(y~rhs,tau=taus[i], method = "br") # lm(y ~ -1 + ., data = datamat)
