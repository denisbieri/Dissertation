TVECM.exo<-function(data, thresh, nthresh=1, thresh1, thresh2, lag=1, trim=0.15, regimespecific=c("All","ECT"), const=c("coint", "TVECM", "none"), plot=TRUE, trace=FALSE){
  ...
  if(nthresh==1 && regimespecific=="ECT"){  
    {
      cat ("Threshold VECM Estimates", "\n")
      cat ("\n")
      cat ("Threshold Estimate            ", bestThresh, "\n")
      cat ("Cointegrating beta Estimate   ", -bestBeta, "\n")
      if(const=="coint"){
        cat ("Cointegrating const. Estimate ", -cointconst, "\n")
      }
      ...
      PvalL <- format(Pval[,-2],digits=4)
      for (j in 1:ncol(BbestL)) cat(" ",BbestL[1,j],"  (",PvalL[1,j],")\n")
      cat ("\n")
      cat ("  Equation 2 (t-statistic in parentheses)","\n")
      for (j in 1:ncol(BbestL)) cat(" ",BbestL[2,j],"  (",PvalL[2,j],")\n")
      cat ("\n")
      cat ("High Regime", "\n")
      ...
      PvalH <- format(Pval[,-1],digits=4)
      for (j in 1:ncol(BbestH)) cat(" ",BbestH[1,j],"  (",PvalH[1,j],")\n")
      cat ("\n")
      cat ("  Equation 2 (t-statistic in parentheses)","\n")
      for (j in 1:ncol(BbestH)) cat(" ",BbestH[2,j],"  (",PvalH[2,j],")\n")
      cat ("\n")
    }
  }
